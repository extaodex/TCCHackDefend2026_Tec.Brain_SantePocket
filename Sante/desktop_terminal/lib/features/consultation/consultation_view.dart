...
class _ConsultationViewState extends State<ConsultationView> {
  ...
  Widget _buildPatientHeader(BuildContext context, String name, String firstName, Map<String, dynamic> record) {
    ...
    if (dobVal != null && dobVal.toString().isNotEmpty) {
      try {
        final parts = dobVal.toString().split('/');
        if (parts.length == 3) {
    ...
    final dobLabel = dobVal ?? 'N/A';
    final sexe = record['sexe'] ?? 'N/A';
    final taille = record['taille'] ?? 'N/A';
    final poids = record['poids'] ?? 'N/A';
    final tension = record['tension'] ?? 'N/A';
        ...
        child: Row(
          children: [
            CircleAvatar(
              ...
              child: Text(name.isNotEmpty && firstName.isNotEmpty ? name[0] + firstName[0] : '?', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: colorScheme.primary)),
            ),
            const SizedBox(width: 24),
            Expanded(
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      ...
                      _buildBadge(record['groupeSanguin'] ?? record['groupe_sanguin'] ?? '?', Colors.red),
                    ],
                  ),
                  const SizedBox(height: 8),
                  Text(
                    'Né(e) le $dobLabel$ageStr • $sexe',
                    style: TextStyle(color: Colors.blueGrey.shade400, fontSize: 16),
                  ),
                ],
              ),
            ),
            _buildInfoTile('Taille', taille),
            _buildVerticalDivider(),
            _buildInfoTile('Poids', poids),
            _buildVerticalDivider(),
            _buildInfoTile('Tension', tension),
          ],
        ),
      ),
    );
  ...
  Widget _buildMedicalHistory(BuildContext context, Map<String, dynamic> record) {
    final rawHistory = record['antecedents']?.toString() ?? '';
    final List<String> historyItems = rawHistory
        .split(',')
        .map((e) => e.trim())
        ...
        child: Column(
          ...
          children: [
            ...
            if (historyItems.isEmpty)
              const Text('Aucun antécédent déclaré par le patient.', style: TextStyle(color: Colors.grey, fontStyle: FontStyle.italic))
            else
              ...historyItems.map((item) => _buildHistoryItem(item, 'Déclaré par le patient')),
          ],
        ),
      ),
    );
