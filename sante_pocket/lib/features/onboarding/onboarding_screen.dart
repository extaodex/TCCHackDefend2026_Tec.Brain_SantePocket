...
class _OnboardingScreenState extends ConsumerState<OnboardingScreen> {
  ...
  void _nextPage() {
    if (_currentPage == 0) {
      if (_nomController.text.isEmpty || _prenomController.text.isEmpty || _selectedDate == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(_selectedDate == null
                ? 'Veuillez sélectionner votre date de naissance via le calendrier'
    ...
    if (_currentPage == 1 && _selectedSexe == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Veuillez choisir votre sexe'),
          backgroundColor: AppTheme.emergencyRedLight,
          behavior: SnackBarBehavior.floating,
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
        ),
    ...
    if (_currentPage == 2 && _selectedGroupe == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Veuillez choisir votre groupe sanguin'),
          backgroundColor: AppTheme.emergencyRedLight,
          behavior: SnackBarBehavior.floating,
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
        ),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text('Information essentielle en cas d\'urgence médicale.',
            textAlign: TextAlign.center,
            style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: Colors.white54),
          ),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 40),
          _buildContinueButton('Créer mon dossier', _finishOnboarding, isPrimary: true),
        ],
      ),
    );
  }
