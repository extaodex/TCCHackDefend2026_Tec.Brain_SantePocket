                        ...
                        child: ElevatedButton.icon(
                          onPressed: () async {
                            ...
                            for (final idx in selectedIndices) {
                              ...
                              final phone = contact.phones.first.number;
                              try {
                                await ref.read(contactsUrgenceProvider.notifier).addContact(
                                  contact.displayName,
                                  'Contact',
                                  phone,
                                );
                                ...
                                if (e.toString().contains('déjà enregistré')) {
                                  duplicateCount++;
                                }
                              }
                            }
                            ...
                            if (context.mounted) {
                              ...
                              String message = '✅ $addedCount contact(s) ajouté(s)';
                              if (duplicateCount > 0) {
                                message += ' ($duplicateCount déjà existant(s))';
                              }
                              
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(message),
                                  backgroundColor: addedCount > 0 ? AppTheme.validatedGreenLight : AppTheme.pendingOrangeLight,
                                  behavior: SnackBarBehavior.floating,
