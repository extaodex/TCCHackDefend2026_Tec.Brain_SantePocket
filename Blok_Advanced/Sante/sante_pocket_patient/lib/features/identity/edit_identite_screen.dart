...
class _EditIdentiteScreenState extends ConsumerState<EditIdentiteScreen> {
  ...
  List<String> _antecedentsList = [];

  String _sexeReadOnly = '';
  bool _isGroupeSanguinReadOnly = false;
  ...
  void initState() {
    ...
    _isGroupeSanguinReadOnly = (patient?.groupeSanguin ?? '').isNotEmpty;
    ...
    _allergiesList = (patient?.allergies ?? '')
        .split(',')
        .map((e) => e.trim())
        .where((e) => e.isNotEmpty)
        .toList();
    _antecedentsList = (patient?.antecedents ?? '')
        .split(',')
        .map((e) => e.trim())
        .where((e) => e.isNotEmpty)
        .toList();
  ...
  Future<void> _sauvegarder() async {
    if (_formKey.currentState!.validate()) {
      ...
      // Confirmation pour le groupe sanguin s'il est renseigné pour la première fois
      final initialGroupe = patient?.groupeSanguin ?? '';
      final nouveauGroupe = _groupeSanguinController.text.trim();
      
      if (initialGroupe.isEmpty && nouveauGroupe.isNotEmpty) {
        final confirmer = await showDialog<bool>(
          context: context,
          builder: (context) => AlertDialog(
            title: const Text('Confirmer le groupe sanguin'),
            content: Text('Vous avez saisi : $nouveauGroupe.\n\nAttention, une fois enregistré, le groupe sanguin ne pourra plus être modifié pour des raisons de sécurité médicale.'),
            actions: [
              TextButton(
      ...
      final updatedPatient = Patient(
        ...
        antecedents: _antecedentsList.join(', '),
      );
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              if (_isGroupeSanguinReadOnly)
                _buildReadOnlyField('Groupe Sanguin (non modifiable)', _groupeSanguinController.text,
                  icon: Icons.bloodtype_rounded,
                  iconColor: AppTheme.emergencyRedLight,
                )
              else
                _buildTextField('Groupe Sanguin', _groupeSanguinController, isRequired: false),

              const SizedBox(height: 16),
              _buildTextField('Taille (ex: 182 cm)', _tailleController, isRequired: false),
              const SizedBox(height: 16),
              _buildTextField('Poids (ex: 78 kg)', _poidsController, isRequired: false),
              const SizedBox(height: 16),
              _buildTextField('Tension (ex: 12/8)', _tensionController, isRequired: false),
              const SizedBox(height: 24),

              // ── Antécédents (Chips) ──
              _buildChipSection('Antécédents / Maladies', _antecedentsList, Icons.history_edu_rounded, AppTheme.primaryBlueLight),
              const SizedBox(height: 24),

              // ── Allergies (Chips) ──
              _buildChipSection('Allergies', _allergiesList, Icons.do_not_disturb_on_rounded, AppTheme.emergencyRedLight),
  ...
  Widget _buildChipSection(String label, List<String> items, IconData icon, Color color) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
          children: [
            ...
            const Spacer(),
            TextButton.icon(
              onPressed: () => _showAddItemDialog(label, items),
        ...
        if (items.isEmpty)
          Padding(
            ...
            child: Text('Aucun élément renseigné', style: TextStyle(color: Colors.grey.shade400, fontSize: 13, fontStyle: FontStyle.italic)),
          )
        else
          Wrap(
            spacing: 8,
  ...
  Widget _buildTextField(String label, TextEditingController controller, {int maxLines = 1, bool isRequired = true}) {
    return TextFormField(
      controller: controller,
      maxLines: maxLines,
      validator: isRequired ? (value) => value!.isEmpty ? 'Ce champ est requis' : null : null,
      decoration: InputDecoration(
        labelText: label,
        filled: true,
