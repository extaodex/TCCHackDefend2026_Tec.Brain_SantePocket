...
class DiscoveryService {
  ...
  Future<void> startDiscovery({int udpPort = kDefaultUdpPort}) async {
    ...
    _myUuid = await UserProfileService.getDoctorUuid();

    // 2. Récupérer le nom de l'appareil
    _deviceName = await UserProfileService.getDoctorName() ?? await _getDeviceName();
    ...
    try {
      ...
      _socket!.listen(_handlePacket, onError: (_) => stopDiscovery());
