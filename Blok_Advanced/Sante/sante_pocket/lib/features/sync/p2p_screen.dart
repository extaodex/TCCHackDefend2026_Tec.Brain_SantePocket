...
class _P2PScreenState extends ConsumerState<P2PScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _importMshFile(ReceivedFile received) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        backgroundColor: const Color(0xFF1E293B),
        title: const Text('Mise à jour du dossier', style: TextStyle(color: Colors.white)),
        content: Text(
          'Voulez-vous mettre à jour votre carnet de santé avec les données reçues de ${received.senderName} ?\n\nCela inclura les notes et ordonnances du médecin.',
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(backgroundColor: AppTheme.primaryBlueLight),
    ...
    if (confirmed == true) {
      try {
        await SyncService.importMshArchive(received.path);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('✅ Carnet de santé mis à jour avec succès !'),
            ...
            children: [
              ...
              if (isMsh)
                IconButton(
                  ...
                  onPressed: () => _importMshFile(received),
                ),
            ],
          ),
        );
